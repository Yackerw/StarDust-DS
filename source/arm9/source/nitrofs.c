...
//---------------------------------------------------------------------------------
	...
	if ( __system_argv->argvMagic == ARGV_MAGIC && __system_argv->argc >= 1 ) {
		if ( strncmp(__system_argv->argv[0],"fat",3) == 0 || strncmp(__system_argv->argv[0],"sd",2) == 0) {
			if (fatInitDefault()) {
				ndsFileFD = fopen(__system_argv->argv[0], "rb");
				if (ndsFileFD != NULL) {
					nitroInit = true;
...
//---------------------------------------------------------------------------------
	...
	} else if(ndsFileFD!=NULL) { //read from ndsfile

		if(ndsFileLastpos!=*npos)
			fseek(ndsFileFD, *npos, SEEK_SET);

		len=fread(ptr, 1, len, ndsFileFD);
